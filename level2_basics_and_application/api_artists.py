...
При открытии файла для записи на Windows по умолчанию используется кодировка CP1251, в то время как для записи
имен на сайте используется кодировка UTF-8, что может привести к ошибке при попытке записать в файл имя с необычными
...
client_id = '81492e56621d80919cdd'
client_secret = '4f9fc9286782f68ac0f42bd3bbb1afae'

# инициируем запрос на получение токена

token_response = requests.post("https://api.artsy.net/api/tokens/xapp_token",
                               data={
                                   "client_id": client_id,
                                   "client_secret": client_secret
                               })
...
headers = {"X-Xapp-Token": token}
# инициируем запрос с заголовком
artist_file_path = path.join(".", "file_samples", "input_artists.txt")
artist_list = open(artist_file_path).readlines()
artist_response_data = []
for artist in artist_list:
    artist_url = f'https://api.artsy.net/api/artists/{artist.rstrip()}'
    artist_response = requests.get(artist_url, headers=headers)

    # разбираем ответ сервера
    artist_data = json.loads(artist_response.text)
    artist_response_data.append(artist_data)


def sort_artist(artist):
    return (artist["birthday"], artist["sortable_name"])


artist_response_data.sort(key=sort_artist)

for new_artist in artist_response_data:
    print(new_artist["sortable_name"])
